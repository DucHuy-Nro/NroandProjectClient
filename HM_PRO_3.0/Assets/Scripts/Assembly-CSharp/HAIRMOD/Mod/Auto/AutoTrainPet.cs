    ...
    internal class AutoTrainPet
    {
        ...
        internal static void Update()
        {
            ...
            if (mSystem.currentTimeMillis() - delayCheckPet < 3000)
                return;
            if (GameCanvas.gameTick % (30 * Time.timeScale) != 0 || HM9rXController.gI.IsActing)
                return;
            AutoPick();
            ...
            if (isGoHomeGetMorePean && !isMagicTreeOutOfPean && GameCanvas.gameTick % (60 * Time.timeScale) == 0)
            {
                ...
                {
                    if (!HM9rXController.gI.IsActing && GameScr.hpPotion == 0)
                        HM9rXController.start(Char.myCharz().cgender + 21);
                }
                else
                {
                    ...
                    if ((GameScr.gI().magicTree.isPeasEffect || GameScr.gI().magicTree.isUpdateTree || GameScr.gI().magicTree.currPeas == 0) && !HM9rXController.gI.IsActing)
                        HM9rXController.start(mapZoneGoBack.Key);
                }
                if (TileMap.mapID == mapZoneGoBack.Key && !HM9rXController.gI.IsActing)
                {
        ...
        static MenuValues[] getMenuValues()
        {
            return new MenuValues[2]
            {
                new MenuValues(new MenuValuesConfig()
                {
                    ID = "Set_AutoTrainPet",
                    Title = Strings.setAutoTrainPetTitle,
                    Values = Strings.setAutoTrainPetChoices,
                    GetValueFunc = () => (int)Mode,
                    SetValueAction = value => SetState((int)value),
                ...
                new MenuValues(new MenuValuesConfig()
                {
                    ...
                    GetValueFunc = () => (int)ModeAttackWhenNeeded,
                    SetValueAction = value => SetAttackState((int)value),
                    RMSName = "auto_pet_mode",
                    GetIsDisabled = () => Mode <= AutoTrainPetMode.Disabled,
        ...
        {
            isFirstTimeCheckPet = true;
            foreach (var menu in getMenuValues())
            {
        ...
        internal static void showMenu()
        {
            ...
            foreach(var menu in getMenuValues())
            {
                if(menu  != null)
                {
                    myVector.addElement(new Command(menu.getSelectedValue() + "\n" + $"[{(int)menu.GetValueFunc()}]", () => menu.action()));
                }
            }
