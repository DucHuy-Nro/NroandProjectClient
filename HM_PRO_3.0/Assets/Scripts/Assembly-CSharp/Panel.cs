...
public class Panel : IActionListener, IChatable
{
    ...
    public static List<string> listPassiveSkillInfo = new List<string>();
        ...
        for (int i = 0; i < currentListLength; i++)
        {
            ...
            if (item.itemOption != null)
            {
                for (int k = 0; k < item.itemOption.Length; k++)
                {
                    ...
                    {
                        if (item.itemOption[k].param > 0)
                            paintItemStar(g, item.itemOption[k].param.ToString(), num + 125, num2);
                    }
    ...
    {
        if (CharTemplate._Caro_Inventory)
        {
            updateKeyScrollViewCaro();
            return;
        }
    ...
    private void setTabInventory(bool resetSelect)
    {
        if (CharTemplate._Caro_Inventory)
        {
            setTabInventoryCaro(resetSelect);
            return;
        }
        ITEM_HEIGHT = 24;
                    ...
                    if (item.buyCoin != 0 || item.buyGold != 0)
                    {
                        ...
                        if (typeShop == 2 && currentTabIndex <= 3 && !Equals(GameCanvas.panel2))
                        {
                            ...
                            else
                            {
                                if (item.buyCoin > 0)
                                {
                                    g.drawImage(imgXu, num2 + num4 - 7, num3 + 7, 3); //hong ngoc
            ...
            if (item != null)
            {
                ...
                if (item.itemOption != null)
                {
                    for (int k = 0; k < item.itemOption.Length; k++)
                    {
                        ...
                        {
                            if (item.itemOption[k].param > 0)
                                paintItemStar(g, item.itemOption[k].param.ToString(), num3 + 125, num4);
                        }
                        if (item.itemOption[k].optionTemplate.id == 41)
                        {
                            if (item.itemOption[k].param == 1)
                ...
                for (int j = 0; j < item.itemOption.Length; j++)
                {
                    ...
                    else if (item.itemOption[j].optionTemplate.id == 107)
                    {
                        if (item.itemOption[j].param > 0)
                            paintItemStar(g, item.itemOption[j].param.ToString(), num2 + 125, num3);
                    }
                }
            }
        ...
        else
        {
            ...
            for (int i = 0; i < currentListLength; i++)
            {
                ...
                if (!archivement.isFinish)
                {
                    ...
                    mFont.tahoma_7_green.drawString(g, archivement.money + " " + mResources.RUBY, num + num3 - 5, num2, mFont.RIGHT); //goldbar
    ...
    #region Caro Inven
    private void updateKeyScrollViewCaro()
    {
        if (currentListLength <= 0)
        {
            return;
        ...
        if (GameCanvas.isPointerDown)
        {
            ...
            {
                var n =  (GameCanvas.panel2 == null && type == 0 && currentTabIndex == 1) || type == 7 || type == 13;
                pointerDownTime++;
                if (pointerDownTime > 5 && pointerDownFirstX == GameCanvas.py && !isDownWhenRunning)
                {
                    pointerDownFirstX = -1000;
                    selected = (cmtoY + GameCanvas.py - (yScroll)) / (ITEM_HEIGHT + (n ?  1 : 0));
                    if (selected >= Char.myCharz().arrItemBody.Length)
                    {
                        if ((GameCanvas.panel2 == null && type == 0 && currentTabIndex == 1) || type == 7 || type == 13)
                        {
                            int space = selected - Char.myCharz().arrItemBody.Length;
                            selected = selected + (GameCanvas.px - (xScroll + 2)) / 34 + space * 4;
                        }
                    }
                    if (selected >= currentListLength)
                    {
        ...
        var n2 = (GameCanvas.panel2 == null && type == 0 && currentTabIndex == 1) || type == 7 || type == 13;
        justRelease = true;
        int i2 = GameCanvas.py - pointerDownLastX[0];
        GameCanvas.isPointerJustRelease = false;
        if (Res.abs(i2) < 20 && Res.abs(GameCanvas.py - pointerDownFirstX) < 20 && !isDownWhenRunning)
        {
            ...
            cmtoY = cmy;
            pointerDownFirstX = -1000;
            selected = (cmtoY + GameCanvas.py - (yScroll)) / (ITEM_HEIGHT + (n2 ? 1 : 0));
            if (selected >= Char.myCharz().arrItemBody.Length)
            {
                if ((GameCanvas.panel2 == null && type == 0 && currentTabIndex == 1) || type == 7 || type == 13)
                {
    ...
    private void setTabInventoryCaro(bool select)
    {
        try
        {
            this.ITEM_HEIGHT = 26;
            ...
            int rowsForBag = (arrItemBag.Length + 4) / 5;
            this.cmyLim = ((arrItemBody.Length + Mathf.Max(5, rowsForBag)) * (this.ITEM_HEIGHT + 1)) - this.hScroll;
    ...
    private void paintInventoryCaro(mGraphics g)
    {
        try
        {
            Item[] arrItemBody = Char.myCharz().arrItemBody;
            Item[] arrItemBag = Char.myCharz().arrItemBag;
            int sum = arrItemBag.Length + arrItemBody.Length;
            g.setColor((sum > arrItemBody.Length) ? 14337964 : 16711680);
            ...
            for (int i = 0; i < arrItemBody.Length + arrItemBag.Length; i++)
            {
                ...
                int num = i;
                int num2 = i - arrItemBody.Length;
                int num3 = this.xScroll + 34;
                int num4 = this.yScroll + i * (this.ITEM_HEIGHT + 1);
                int num5 = this.wScroll - 34;
                int num6 = this.ITEM_HEIGHT - 1;
                int num7 = this.xScroll;
                int num8 = this.yScroll + i * (this.ITEM_HEIGHT + 1);
                int num9 = 32;
                ...
                if (flag)
                {
                    if (num4 - this.cmy <= this.yScroll + this.hScroll)
                    {
                        if (num4 - this.cmy >= this.yScroll - this.ITEM_HEIGHT)
                        {
                            ...
                            g.fillRect(num3, num4, num5, num6, 4);
                            g.setColor((i != this.selected) ? ((!flag) ? 11837316 : 9993045) : 9541120);
                            g.fillRect(num7, num8, num9, num10, 4);
                            if (item != null && item.isSelect && GameCanvas.panel.type == 12)
                            {
                            ...
                            if (item != null)
                            {
                                ...
                                mFont.tahoma_7_green2.drawString(g, $"[{item.template.id}]" + item.template.name, num3 + 5, num4 + 1, 0);
                                string text = string.Empty;
                                if (item.itemOption != null)
                                {
                                    for (int l = 0; l < item.itemOption.Length; l++)
                                    {
                                        ...
                                        else if (item.itemOption[l].optionTemplate.id == 107)
                                        {
                                            if (item.itemOption[l].param > 0)
                                                paintItemStar(g, item.itemOption[l].param.ToString(), num3 + 125, num4);
                                        }
                                ...
                                SmallImage.drawSmallImage(g, (int)item.template.iconID, num7 + num9 / 2 + 1, num8 + num10 / 2 , 0, 3);
                                if (item.quantity > 1)
                                {
                                    mFont.tahoma_7b_yellow.drawString(g, item.quantity.ToString(), num7 + num9 - 15, num8 + 6, 0);
                                }
                            }
                        }
                ...
                {
                    int j = i - arrItemBody.Length;
                    int x = this.xScroll + 2  + (j % 5) * (34);
                    int w = 32;
                    int y = this.yScroll + ((ITEM_HEIGHT + 1) * arrItemBody.Length) + ((j / 5) * (ITEM_HEIGHT + 1));
                    int h = ITEM_HEIGHT - 1;
                    ...
                    if (item != null)
                    {
                        ...
                        if (item.quantity > 1)
                        {
                            mFont.tahoma_7b_yellow.drawString(g, NinjaUtil.getMoneys(item.quantity), x + w, y + h - mFont.tahoma_7b_yellow.getHeight(), mFont.RIGHT, mFont.tahoma_7b_dark);
                        }
                    }
                }
            }
        ...
        for (int i = 0; i < arrItemBody.Length + arrItemBag.Length; i++)
        {
            ...
            if (item.itemOption != null)
            {
                for (int j = 0; j < item.itemOption.Length; j++)
                {
                    ...
                    {
                        if (item.itemOption[j].param > 0)
                            paintItemStar(g, item.itemOption[j].param.ToString(), num3 + 125, num4);
                    }
                }
            }
    ...
    private void paintTab(mGraphics g)
    {
        ...
        for (int i = 0; i < currentTabName.Length; i++)
        {
            ...
            mFont mFont2 = currentTabName[i][1] == "Game" ? mFont.tahoma_7_blue : ((i != currentTabIndex) ? mFont.tahoma_7_grey : mFont.tahoma_7_green2);
    ...
    {
        mFont.tahoma_7b_white.drawString(g, "Ngọc Rồng GOTECKS", 60, 4, mFont.LEFT, mFont.tahoma_7b_dark);
    ...
    private void doSpeacialSkill()
    {
        ...
        myVector.addElement(new Command("Mở Tùy Chọn", AutoNoiTai.gI(), 8, Char.myCharz().infoSpeacialSkill[currentTabIndex][selected]));
    ...
    private void doFireShop()
    {
        ...
        if (currItem != null)
        {
            ...
            if (type != 4 && type != 17)
            {
                myVector.addElement(new Command("Auto Mua", () => AutoItem.gI().addItemBuys(new AutoItem(currItem.template.id, (sbyte)currItem.template.id, -1, currItem.buyGold == 1, currItem.buyGold != 1))));
            }
            GameCanvas.menu.startAt(myVector, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
            addItemDetail(currItem);
        }
    ...
    private void doFireInventory()
    {
        ...
        else
        {
            ...
            if (currItem != null)
            {
                ...
                {
                    if (!AutoItem.checkList(currItem.template.id))
                    {
                        if (AutoItem.ItemMustBeUse(currItem))
                            myVector.addElement(new Command("Auto Sử Dụng", () => AutoItem.gI().addItemUses(currItem.template.id, (sbyte)currItem.indexUI)));
                    }
                    else
                    {
                        myVector.addElement(new Command("Ngưng Sử Dụng", () => AutoItem.gI().removeItemUses(currItem.template.id)));
                    }
                    if (position == 0)
                    {
                ...
                GameCanvas.menu.startAt(myVector, X,  ITEM_HEIGHT + yScroll);
                addItemDetail(currItem);
            }
            else
            {
        ...
        if (selected == 0 || selected == 1 || selected == 2 || selected == 3 || selected == 4 || selected == 5)
        {
            ...
            if (selected == 0)
            {
                ...
                if (cTiemNang >= 10 * (2 * (cHPGoc + num2) + 180) / 2 && cTiemNang < 100 * (2 * (cHPGoc + num2) + 1980) / 2)
                {
                    ...
                    myVector.addElement(new Command("Cộng Nhanh", () => new AutoPoint().perform(1, null)));
                    GameCanvas.menu.startAt(myVector, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                ...
                if (cTiemNang >= 100 * (2 * (cHPGoc + num2) + 1980) / 2)
                {
                    ...
                    myVector2.addElement(new Command("Cộng Nhanh", () => new AutoPoint().perform(1, null)));
                    GameCanvas.menu.startAt(myVector2, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2(selected);
                }
            }
            if (selected == 1)
            {
                ...
                if (cTiemNang >= 10 * (2 * (cMPGoc + num2) + 180) / 2 && cTiemNang < 100 * (2 * (cMPGoc + num2) + 1980) / 2)
                {
                    ...
                    myVector3.addElement(new Command("Cộng Nhanh", () => new AutoPoint().perform(2, null)));
                    GameCanvas.menu.startAt(myVector3, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2(selected);
                }
                if (cTiemNang >= 100 * (2 * (cMPGoc + num2) + 1980) / 2)
                {
                    ...
                    myVector4.addElement(new Command("Cộng Nhanh", () => new AutoPoint().perform(2, null)));
                    GameCanvas.menu.startAt(myVector4, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2(selected);
                }
            }
            if (selected == 2)
            {
                ...
                if (cTiemNang >= 10 * (2 * cDamGoc + 9) / 2 * Char.myCharz().expForOneAdd && cTiemNang < 100 * (2 * cDamGoc + 99) / 2 * Char.myCharz().expForOneAdd)
                {
                    ...
                    myVector5.addElement(new Command("Cộng Nhanh", () => new AutoPoint().perform(3, null)));
                    GameCanvas.menu.startAt(myVector5, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                    addSkillDetail2(selected);
                }
                if (cTiemNang >= 100 * (2 * cDamGoc + 99) / 2 * Char.myCharz().expForOneAdd)
                {
                    ...
                    myVector6.addElement(new Command("Cộng Nhanh", () => new AutoPoint().perform(3, null)));
            ...
            if (selected == 3)
            {
                ...
                myVector7.addElement(new Command("Cộng Nhanh", () => new AutoPoint().perform(4, null)));
