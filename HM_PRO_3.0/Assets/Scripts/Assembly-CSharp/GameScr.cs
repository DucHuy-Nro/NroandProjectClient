...
public class GameScr : mScreen, IChatable
{
	...
	public bool isAttack()
	{
		...
		if (Char.myCharz().mobFocus != null)
		{
			...
			int num3 = MathTea.abs(Char.myCharz().cx - Char.myCharz().mobFocus.getX());
			int num4 = MathTea.abs(Char.myCharz().cy - Char.myCharz().mobFocus.getY());
			Char.myCharz().cvx = 0;
		...
		if (Char.myCharz().npcFocus != null)
		{
			...
			int num8 = MathTea.abs(Char.myCharz().cx - Char.myCharz().npcFocus.cx);
			int num9 = MathTea.abs(Char.myCharz().cy - Char.myCharz().npcFocus.cy);
			if (num9 > 40)
			{
		...
		if (Char.myCharz().charFocus != null)
		{
			...
			int num11 = MathTea.abs(Char.myCharz().cx - Char.myCharz().charFocus.cx);
			int num12 = MathTea.abs(Char.myCharz().cy - Char.myCharz().charFocus.cy);
	...
	public override void updateKey()
	{
        ...
        if (CustomPanel.isShow)
        {
            CustomPanel.updateTouch();
            return;
        }
            HM9r329.onUpdateTouchGameScr();
	...
	{
		if (isAnalog == 1 || gamePad.disableCheckDrag() || CustomPanel.isShow)
        {
			return;
		}
	...
	{
		if (gamePad.disableClickMove() || CustomPanel.isShow)
        {
			return;
		}
		Char.myCharz().cancelAttack();
	...
	private void pickItem()
	{
		...
		int num = MathTea.abs(Char.myCharz().cx - Char.myCharz().itemFocus.x);
		int num2 = MathTea.abs(Char.myCharz().cy - Char.myCharz().itemFocus.y);
		if (num <= 40 && num2 < 40)
		{
			GameCanvas.clearKeyHold();
			GameCanvas.clearKeyPressed();
	...
	private void paintInfoBar(mGraphics g)
	{
		...
		else
		{
			...
			int n = 0;
            if (Char.myCharz().charFocus != null && Char.myCharz().charFocus.isBOSS())
            {
                int w = 200;
                int h = 15;
                int x = GameCanvas.hw - w / 2;
                int y = 60;
                long hp = 0L, hpMax = 0L;
                if (Char.myCharz().charFocus != null)
                {
                    hp = Char.myCharz().charFocus.cHP;
                    hpMax = Char.myCharz().charFocus.cHPFull;
                    n = (int)(Char.myCharz().charFocus.cHP * w / Char.myCharz().charFocus.cHPFull);
                }
                if (n - 3 < 0)
                {
                    n = 0;
                ...
                g.fillRect(x + 2, y + 2, n - 4, h - 4, 10);
                mFont.tahoma_7b_white.drawStringBd(g, $"{(hp <= int.MaxValue ? NinjaUtil.getMoneys(hp) :  Res.formatNumber2(hp))}/{Res.formatNumber2(hpMax)}", x + w / 2, y + h / 2 - mFont.tahoma_7b_white.getHeight() / 2, 3, mFont.tahoma_7);
            }
        }
		g.translate(-g.getTranslateX(), -g.getTranslateY());
		if (isVS() && secondVS > 0)
	...
	public void updateSS()
	{
		if (indexMenu != -1)
		{
			...
			if (MathTea.abs(cmtoYSK - cmySK) < 15 && cmySK < 0)
			{
				cmtoYSK = 0;
			}
			if (MathTea.abs(cmtoYSK - cmySK) < 15 && cmySK > cmyLimSK)
			{
				cmtoYSK = cmyLimSK;
			}
		}
	...
	public void chatVip(string chatVip)
	{
        ...
        if (chatVip.StartsWith("BOSS ") || chatVip.StartsWith("Boss "))
        {
            Boss._ListBosses.Add(new Boss(chatVip.Replace("Boss", "BOSS")));
            if (Boss._ListBosses.Count > 5)
                Boss._ListBosses.RemoveAt(0);
        }
