...
public class GameCanvas : IActionListener
{
	...
	public void initGame()
	{
		...
		//if(LoginScr.imgTitle != null && mGraphics.zoomLevel != 4)
		//{
		//	Utils.resizeImage(LoginScr.imgTitle, 4);
		//}
        ...
        try
        {
            ...
            else
            {
                ...
                if (layerSpeed[num] != 0)
                {
                    ...
                    {
                        if (currentScreen is ServerListScreen || currentScreen is ServerScr || currentScreen is LoginScr)
                        {
                            //g.reset();
                            if (num == 0)
                            {

                                var region = new Rect(0, yb[num], bgW[num], bgH[num]);
                                Matrix4x4 matrix4X4 = GUI.matrix;
                                float xxx = 1.1f;
                                GUIUtility.ScaleAroundPivot(new Vector2(1, 1.2f + (0.05f * mGraphics.zoomLevel)), new Vector2(region.x + region.width / 2, region.y + region.height / 2));

                                g.drawImage(imgBG[num], i, yb[num]);
                            ...
                            {
                                g.drawImage(imgBG[num], i, yb[num], 0);
                            }
                        }
                        else
                        {
	...
	public static void paintShukiren(int x, int y, mGraphics g)
	{
        ...
        g.drawRegion(imgShuriken, 0, GameCanvas.gameTick % 9 * (mGraphics.getImageHeight(imgShuriken) / 9), imgShuriken.getWidth(), imgShuriken.getHeight() / 9, 0, x, y, mGraphics.HCENTER | mGraphics.VCENTER);
